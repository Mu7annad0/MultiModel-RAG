...
class GenerationClient:
    def __init__(self, settings: dict, provider: Optional[str] = "openai"):
        ...
        self.provider: Optional[Union[OpenAIProvider, GeminiProvider, DeepSeekProvider]] = None
        self.set_model(provider)

    def set_model(self, provider: str, model_id: Optional[str] = None, api_key: Optional[str] = None):
        provider = provider.lower()

        if provider == "openai":
            model_id = model_id or getattr(self.settings, "OPENAI_MODEL_ID", "gpt-5-mini")
            ...
            model_id = model_id or getattr(self.settings, "GEMINI_MODEL_ID", "gemini-3-flash-preview")
            ...
            model_id = model_id or getattr(self.settings, "DEEPSEEK_MODEL_ID", "deepseek/deepseek-r1-0528:free")
    ...
    def answer(self, prompt: str, chat_history: Optional[List[Dict]] = None) -> str:
        if self.provider is None:
            raise RuntimeError("Provider not set. Call set_model() first.")
        return self.provider.generate(prompt, chat_history or [])
    
    def answer_stream(self, prompt: str, chat_history: Optional[List[Dict]] = None):
        if self.provider is None:

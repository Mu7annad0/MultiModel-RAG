...
class QrantVectorDB:
    ...
    async def create_collection(
        ...
        try:
            ...
            if do_reset:
                self.logger.info(
                    f"Reset flag is True, deleting existing collection if exists"
                )
            ...
            self.logger.error(
                f"Error creating collection {collection_name}: {str(e)}", exc_info=True
            )
            return False
    ...
    async def insert(
        ...
        try:
            self.logger.info(
                f"Inserting {len(texts)} documents into collection: {collection_name}"
            )
            ...
            for i in range(0, len(texts), batch_size):
                ...
                batch_records = [
                    models.PointStruct(
                        ...
                        payload={
                            ...
                            if batch_metadata and x < len(batch_metadata)
                            else {},
                        },
                    )
                ...
                try:
                    ...
                    self.logger.info(
                        f"Successfully inserted batch {i // batch_size + 1} ({len(batch_records)} documents)"
                    )
                except Exception as e:
                    self.logger.error(
                        f"Error while inserting batch {i // batch_size + 1}: {str(e)}"
                    )
                    return False
